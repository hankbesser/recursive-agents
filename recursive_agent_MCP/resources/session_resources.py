...
async def resource_session_suggestions(
    session_id: str,
    companion_type: str = "generic"
) -> Dict[str, Any]:
    """Get AI-generated suggestions for next actions.
    
    URI: resource://sessions/{session_id}/{companion_type}/suggestions
    
    This would integrate with PhaseIntelligenceMiddleware to provide
    suggestions based on the current state.
    """
    comp = session_manager.get_companion(session_id, companion_type)
    current_slot = comp.run_log[-1] if comp.run_log else None
    ...
    if critiques > revisions:
        suggestion = "Ready to revise based on critique"
        confidence = "high"
    elif critiques == revisions and critiques >= 3:
        suggestion = "Consider finalizing after 3 iterations"
        confidence = "medium"
    elif critiques == revisions:
        suggestion = "You can critique again or finalize"
        confidence = "medium"
    else:
        suggestion = "Continue with the process"
        confidence = "low"
    
    return {
        "suggestion": suggestion,
        "confidence": confidence,
        "iteration_count": max(critiques, revisions),
        "time_estimate": "2-5 minutes for next phase"
    }